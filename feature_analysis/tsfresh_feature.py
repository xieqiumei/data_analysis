...
if __name__ == '__main__':
    ...
    df_train = extract_features(df_train, column_id="series_id", chunksize=7, default_fc_parameters=None,
                                column_sort="measurement_number", impute_function=impute, n_jobs=7)
    df_train.to_csv('train_feature.csv', index=False)
    del df_train

    df_test = pd.read_csv('../input/X_test.csv')
    df_test.drop(['row_id'], axis=1, inplace=True)
    df_test = extract_features(df_test, column_id="series_id", chunksize=7, default_fc_parameters=None,
                               column_sort="measurement_number", impute_function=impute, n_jobs=7)
